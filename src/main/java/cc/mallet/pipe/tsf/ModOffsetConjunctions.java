...
public class ModOffsetConjunctions extends Pipe implements Serializable {
	int[][] conjunctions;
	...
	private static void initStartEndFs() {
		for (int i = 0; i < maxWindowSize; i++) {
			startfs[i] = PropertyList.add("W=<START" + i + ">", 1.0, null);
			endfs[i] = PropertyList.add("W=<END" + i + ">", 1.0, null);
		}
	}
	...
	public ModOffsetConjunctions(boolean includeOriginalSingletons,
			Pattern featureRegex, int[][] conjunctions) {
	...
	public ModOffsetConjunctions(int[][] conjunctions) {
		this(false, conjunctions);
	}
	...
	public Instance pipe(Instance carrier) {
		...
		for (int i = 0; i < tsSize; i++) {
			for (int j = 0; j < conjunctions.length; j++) {
				...
				PropertyList.Iterator[] iters = getOffsetIters(conjunctions, j,
						tsSize, i, oldfs);
				if (iters == null)
					continue;
				...
				for (int ii = 0; ii < iterIndices.length; ii++)
					iterIndices[ii] = -1;
				newfs[i] = makeConjunctions(iters, 0, conjunctions, j, tsSize,
						newfs[i], i, oldfs, iterIndices);
			}
		}
	...
	/**
	 * Is the current feature redundant? The current feature is determined by
	 * the current values in iterIndices, which tells us where we are in each
	 * PropertyList.Iterator. We do this test to ensure we only include the
	 * upper triange of conjunctions.
	 * 
	 * @param conjunctions
	 *            conjunction array
	 * @param j
	 ...
	 * @param iterIndices
	 *            counters for each PropertyList.Iterator
	...
	private boolean redundant(int[][] conjunctions, int j, int[] iterIndices) {
		for (int i = 1; i < iterIndices.length; i++) {
			if (conjunctions[j][i - 1] == conjunctions[j][i]
					&& iterIndices[i] <= iterIndices[i - 1])
				return true;
		}
		return false;
	}
	...
	private void writeObject(ObjectOutputStream out) throws IOException {
		...
		if (size1 != NULL_INTEGER) {
			for (int i = 0; i < size1; i++) {
				size2 = (conjunctions[i] == null) ? NULL_INTEGER
						: conjunctions[i].length;
				out.writeInt(size2);
