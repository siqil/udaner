 ...
 */
public class CRFTrainerByOverlapFeatures extends TransducerTrainer implements
		TransducerTrainer.ByOptimization {
	private static Logger logger = MalletLogger
			.getLogger(CRFTrainerByOverlapFeatures.class.getName());
	...
	public CRFTrainerByOverlapFeatures(CRF crf) {
		this.crf = crf;
	}
	...
	/**
	 * Use this method to specify whether or not factors are added to the CRF by
	 * this trainer. If you have already setup the factors in your CRF, you may
	...
	public CRFOptimizableByLabelLikelihood getOptimizableCRF(
			InstanceList trainingSet, InstanceList unlabeled) {
		if (cachedWeightsStructureStamp != crf.weightsStructureChangeStamp) {
			if (!useNoWeights) {
				((CRFIm) crf).setWeightsDimensionIntersect(new InstanceList[] {
						trainingSet, unlabeled });
			}
			// reallocateSufficientStatistics(); // Not necessary here because
			// it is done in the constructor for OptimizableCRF
			ocrf = null;
	...
	public Optimizer getOptimizer(InstanceList trainingSet,
		...
		getOptimizableCRF(trainingSet, unlabeled); // this will set this.mcrf if
													// necessary
		if (opt == null || ocrf != opt.getOptimizable())
			opt = new LimitedMemoryBFGS(ocrf); // Alternative: opt = new
												// ConjugateGradient (0.001);
		return opt;
	}
	...
	// instance variables?
	// ANSWER: Yes and yes, but you have to use special syntax in the subclass
	// ctor (see mallet-dev archive) -cas

	public boolean trainIncremental(InstanceList training) {
		return train(training, Integer.MAX_VALUE);
	}

	public boolean train(InstanceList trainingSet, InstanceList unlabeled,
			int numIterations) {
		if (numIterations <= 0)
			return false;
		...
		getOptimizableCRF(trainingSet, unlabeled); // This will set this.mcrf if
													// necessary
		getOptimizer(trainingSet, unlabeled); // This will set this.opt if
												// necessary
	...
	/**
	 * Train a CRF on various-sized subsets of the data. This method is
	 * typically used to accelerate training by quickly getting to reasonable
	 * parameters on only a subset of the parameters first, then on
	 * progressively more data.
	 * 
	 * @param training
	 *            The training Instances.
	 * @param numIterationsPerProportion
	 *            Maximum number of Maximizer iterations per training
	 *            proportion.
	 * @param trainingProportions
	 *            If non-null, train on increasingly larger portions of the
	 *            data, e.g. new double[] {0.2, 0.5, 1.0}. This can sometimes
	 *            speedup convergence. Be sure to end in 1.0 if you want to
	 *            train on all the data in the end.
	 * @return True if training has converged.
	 */
	public boolean train(InstanceList training, int numIterationsPerProportion,
		...
		for (int i = 0; i < trainingProportions.length; i++) {
			...
			logger.info("Training on " + trainingProportions[i]
					+ "% of the data this round.");
			if (trainingProportions[i] == 1.0)
				converged = this.train(training, numIterationsPerProportion);
			else
				converged = this.train(
						training.split(new Random(1), new double[] {
								trainingProportions[i],
								1 - trainingProportions[i] })[0],
	 ...
	 * Feature induction is run periodically during training. The features are
	 * added to improve performance on the mislabeled instances, with the
	 * specific scoring criterion given by the {@link FeatureInducer} specified
	 * by <code>gainName</code>
	 * 
	 * @param training
	 *            The training Instances.
	 * @param validation
	 ...
	 *            Number of maximizer iterations between each call to the
	 *            Feature Inducer.
	 * @param numFeatureInductions
	 *            Maximum number of rounds of feature induction.
	 * @param numFeaturesPerFeatureInduction
	 *            Maximum number of features to induce at each round of
	 *            induction.
	 * @param trueLabelProbThreshold
	 *            If the model's probability of the true Label of an Instance is
	 *            less than this value, it is added as an error instance to the
	 *            {@link FeatureInducer}.
	 * @param clusteredFeatureInduction
	 *            If true, a separate {@link FeatureInducer} is constructed for
	 *            each label pair. This can avoid inducing a disproportionate
	 *            number of features for a single label.
	 * @param trainingProportions
	 *            If non-null, train on increasingly larger portions of the data
	 *            (e.g. [0.2, 0.5, 1.0]. This can sometimes speedup convergence.
	 * @param gainName
	 *            The type of {@link FeatureInducer} to use. One of "exp",
	 *            "grad", or "info" for {@link ExpGain}, {@link GradientGain},
	 *            or {@link InfoGain}.
	 * @return True if training has converged.
	 */
	public boolean trainWithFeatureInduction(InstanceList trainingData,
			...
			int numFeatureInductions, int numFeaturesPerFeatureInduction,
			double trueLabelProbThreshold, boolean clusteredFeatureInduction,
			double[] trainingProportions, String gainName) {
		int trainingIteration = 0;
		...
		if (crf.globalFeatureSelection == null) {
			// Mask out all features; some will be added later by
			// FeatureInducer.induceFeaturesFor(.)
			crf.globalFeatureSelection = new FeatureSelection(
					trainingData.getDataAlphabet());
			trainingData.setFeatureSelection(crf.globalFeatureSelection);
		}
		// TODO Careful! If validationData and testingData get removed as
		...
		for (int featureInductionIteration = 0; featureInductionIteration < numFeatureInductions; featureInductionIteration++) {
			...
			logger.info("Feature induction iteration "
					+ featureInductionIteration);

			// Train the CRF
			InstanceList theTrainingData = trainingData;
			if (trainingProportions != null
					&& featureInductionIteration < trainingProportions.length) {
				logger.info("Training on "
						+ trainingProportions[featureInductionIteration]
						+ "% of the data this round.");
				InstanceList[] sampledTrainingData = trainingData
						.split(new Random(1),
								new double[] {
										trainingProportions[featureInductionIteration],
										1 - trainingProportions[featureInductionIteration] });
				theTrainingData = sampledTrainingData[0];
				theTrainingData.setFeatureSelection(crf.globalFeatureSelection); // xxx
																					// necessary?
				logger.info("  which is " + theTrainingData.size()
						+ " instances");
			}
			boolean converged = false;
			if (featureInductionIteration != 0)
				// Don't train until we have added some features
				converged = this.train(theTrainingData,
						numIterationsBetweenFeatureInductions);
			trainingIteration += numIterationsBetweenFeatureInductions;

			logger.info("Starting feature induction with "
					+ crf.inputAlphabet.size() + " features.");

			// Create the list of error tokens, for both unclustered and
			// clustered feature induction
			InstanceList errorInstances = new InstanceList(
					trainingData.getDataAlphabet(),
					trainingData.getTargetAlphabet());
			// This errorInstances.featureSelection will get examined by
			// FeatureInducer,
			...
			for (int i = 0; i < numLabels; i++)
				for (int j = 0; j < numLabels; j++) {
					clusteredErrorInstances[i][j] = new InstanceList(
							trainingData.getDataAlphabet(),
							trainingData.getTargetAlphabet());
					clusteredErrorInstances[i][j]
							.setFeatureSelection(crf.globalFeatureSelection);
			...
			for (int i = 0; i < theTrainingData.size(); i++) {
				...
				SumLattice lattice = crf.sumLatticeFactory.newSumLattice(crf,
						input, (Sequence) null, (Transducer.Incrementor) null,
						(LabelAlphabet) theTrainingData.getTargetAlphabet());
				int prevLabelIndex = 0; // This will put extra error instances
										// in this cluster
				for (int j = 0; j < trueOutput.size(); j++) {
					Label label = (Label) ((LabelSequence) trueOutput)
							.getLabelAtPosition(j);
					assert (label != null);
					// System.out.println
					// ("Instance="+i+" position="+j+" fv="+lattice.getLabelingAtPosition(j).toString(true));
					LabelVector latticeLabeling = lattice
							.getLabelingAtPosition(j);
					double trueLabelProb = latticeLabeling.value(label
							.getIndex());
					int labelIndex = latticeLabeling.getBestIndex();
					// System.out.println
					// ("position="+j+" trueLabelProb="+trueLabelProb);
					if (trueLabelProb < trueLabelProbThreshold) {
						logger.info("Adding error: instance="
								+ i
								+ " position="
								+ j
								+ " prtrue="
								...
								+ (label == latticeLabeling.getBestLabel() ? "  "
										: " *") + " truelabel=" + label
								+ " predlabel="
								+ latticeLabeling.getBestLabel() + " fv="
						...
						clusteredErrorInstances[prevLabelIndex][labelIndex]
								.add(input.get(j), label, null, null);
						clusteredErrorLabelVectors[prevLabelIndex][labelIndex]
								.add(latticeLabeling);
					}
			...
			if (clusteredFeatureInduction) {
				...
				for (int i = 0; i < numLabels; i++) {
					for (int j = 0; j < numLabels; j++) {
						// Note that we may see some "impossible" transitions
						// here (like O->I in a OIB model)
						// because we are using lattice gammas to get the
						// predicted label, not Viterbi.
						// I don't believe this does any harm, and may do some
						// good.
						logger.info("Doing feature induction for "
								+ crf.outputAlphabet.lookupObject(i) + " -> "
								+ crf.outputAlphabet.lookupObject(j) + " with "
								+ clusteredErrorInstances[i][j].size()
								+ " instances");
						if (clusteredErrorInstances[i][j].size() < 20) {
							logger.info("..skipping because only "
									+ clusteredErrorInstances[i][j].size()
									+ " instances.");
							continue;
						}
						...
						for (int k = 0; k < s; k++)
							lvs[k] = (LabelVector) clusteredErrorLabelVectors[i][j]
									.get(k);
						RankedFeatureVector.Factory gainFactory = null;
						if (gainName.equals("exp"))
							gainFactory = new ExpGain.Factory(lvs,
				...
				for (int i = 0; i < numLabels; i++) {
					for (int j = 0; j < numLabels; j++) {
						...
						// Note that this adds features globally, but not on a
						// per-transition basis
						klfi[i][j]
								.induceFeaturesFor(trainingData, false, false);
						if (testingData != null)
							klfi[i][j].induceFeaturesFor(testingData, false,
									false);
					}
				}
				klfi = null;
				...
				if (gainName.equals("exp"))
					...
					gainFactory = new InfoGain.Factory();
				FeatureInducer klfi = new FeatureInducer(gainFactory,
				...
				logger.info("CRF4 FeatureSelection now includes "
						+ crf.globalFeatureSelection.cardinality()
						+ " features");
				klfi = null;
			}
	...
	/**
	 * Sets whether to use the 'some unsupported trick.' This trick is, if
	 * training a CRF where some training has been done and sparse weights are
	 * used, to add a few weights for feaures that do not occur in the tainig
	 * data.
	 * <p>
